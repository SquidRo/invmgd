...
class invmgdRestTest(testing.TestCase):
    ...
    def test_post_feed_box(self):
        """Test the response of /feed_box."""

        test_json = {
            "data": [
                { "item": "K2V4PCB","box_num": 10 },
                { "item": "K2V4PCB","box_num": 20 }
            ]
        }

        self.p1 = mock.patch('util.util_sql.add_stock_box', return_value = 0)

        self.p1.start()
        response = self.simulate_post('/feed_box', json = test_json)
        self.p1.stop()
    ...
    def test_post_pick_box(self):
        """Test the response of /pick_box."""
        ...
        self.p1 = mock.patch('util.util_sql.add_used_box', return_value = 0)

        self.p1.start()
        response = self.simulate_post('/pick_box', json = test_json)
        self.p1.stop()
